    ...
    class Program
    {
        ...
        private class LinqSamples
        {
            ...
            public void DataSetLinq42()
            {
                ...
                idx = 0;
                foreach (var item in products.GroupBy(a => a.Field<string>("Category")))
                {
                    idx++;
                    //var ret = item.AsEnumerable().ToList();
                    item.AsEnumerable().AsParallel().ForAll((a) => { _results.Add(new TestData() { Id = idx, Data = a }); });
                }
            }

            private IList<TestData> _results = new List<TestData>();
